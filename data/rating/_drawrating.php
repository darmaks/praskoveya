...
// Vorobyov Andrey
//$voted=mysql_num_rows(mysql_query("SELECT used_ips FROM $rating_dbname.$rating_tableName WHERE used_ips LIKE '%".$ip."%' AND id='".$id."' AND vote_date=CURRENT_DATE()"));
$voted=mysql_num_rows(mysql_query("SELECT used_ips FROM $rating_dbname.$rating_tableName WHERE used_ips LIKE '%".$ip."%' AND id='".$id."' AND vote_date=CURRENT_DATE() "));
//echo "$voted";
// Vorobyov Andrey
...
if ($static == 'static') {
      ...
      $rater.='     <li class="current-rating" style="width:'.$rating_width.'px;">Текущее '.$rating2.'/'.$units.'</li>';

      for ($ncount = 1; $ncount <= $units; $ncount++) { // loop from 1 to the number of units
           if(!$voted) { // if the user hasn't yet voted, draw the voting stars
              $rater.='<li><a href="db.php?j='.$ncount.'&amp;q='.$id.'&amp;t='.$ip.'&amp;c='.$units.'" title="'.$ncount.' из '.$units.'" class="r'.$ncount.'-unit rater" rel="nofollow">'.$ncount.'</a></li>';
           }
      }
      $ncount=0; // resets the count

      $rater.='  </ul>';
      $rater.='  <p style="text-align: left; margin:0;"';
      if($voted){ $rater.=' class="voted"'; }
      $rater.='>Рейтинг: <strong> '.$rating1.'</strong>/'.$units.' (голосов: '.$count.')';
      $rater.='  </p>';
      $rater.='</div>';
      $rater.='</div>';
      return $rater;
